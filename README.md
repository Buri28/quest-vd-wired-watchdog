...
ただし、**PC 側か Quest 側のアプリがそもそも起動していない場合は、3 回連続を待たずに即座に再起動します。**
「アプリが動いていない」は接触不良のような一過性の現象ではなく確定的な異常だからです。
PC を再起動した直後、HMD の電源を入れ直した直後がこれに当たり、いずれも数秒で復旧します。

`tun` の有無だけでは不十分です。**HMD を起動したまま PC を再起動すると**、Quest 側の VPN は
生き残るため `tun` は残りますが、PC 側にはホストプロセスも `adb reverse` マッピングも存在しません。
この「殻だけが残った状態」を健全と誤判定しないよう、PC 側の 2 条件も必ず確認しています。
